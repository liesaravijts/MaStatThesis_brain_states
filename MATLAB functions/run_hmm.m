function [hmm,Gamma,vpath,state_dynamics,GammaInit,fehist,run_minfe]=run_hmm(subject_list,file_id,params,multiple_runs,K,Data_dir,Work_dir,write_output,nreps,data) 
    % run_hmm Runs the hidden Markov model (HMM).
    %   [hmm,Gamma,vpath,state_dynamics,GammaInit,fehist,run_minfe]=run_hmm(subject_list,file_id,params,multiple_runs,K,Data_dir,Work_dir,write_output,nreps=3(,data))
    %   
    ...
    tic
    disp(['<strong>Running hidden Markov model (HMM)</strong> (subjects = ' num2str(size(subject_list,1)) ', repetitions = ' num2str(nreps) ')'])
    ...
    options.id_mixture = 0; % estimate HMM
    % data options
    ...
    %% run HMM
    [hmm,Gamma,vpath,GammaInit,fehist,state_dynamics] = deal(cell(nreps,1));
    for rep = 1:nreps
        disp(['<strong>HMM run ' num2str(rep) '/' num2str(nreps) '</strong>'])
        % run the model
        [hmm{rep},Gamma{rep},~,vpath{rep},GammaInit{rep},~,fehist{rep}] = hmmmar(f,T,options);
        
        % FC matrix for each state (correlation matrices)
        for s = 1:hmm{rep}.K % loop over states
            [~,hmm{rep}.state(s).FC] = getFuncConn(hmm{rep},s);
        end
        
        % state dynamics
        state_dynamics{rep} = get_state_dynamics(T,vpath{rep},Gamma{rep});
        
        % save results
        if write_output
            HMM_results = struct('rep',rep,'files',char(f),'T',cell2mat(T),'hmm',hmm{rep},'Gamma',Gamma{rep},'vpath',vpath{rep},...
                                 'GammaInit',GammaInit{rep},'fehist',fehist{rep},'state_dynamics',state_dynamics{rep});
            save([Work_dir '/Results/HMM_K' num2str(K) '_rep' num2str(rep,'%02.f') '.mat'],'HMM_results');
        end
    end
    ...
    disp(['<strong>Running hidden Markov model (HMM): Done</strong>  (Time elapsed: ' char(endtime) ')'])
